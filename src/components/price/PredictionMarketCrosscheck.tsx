          ...
          {hasGaps ? (
            <div className="mt-3 space-y-2">
              {data.crossVenue.map((gap) => (
                <div key={`${gap.polymarketUrl}:${gap.kalshiUrl}`} className="rounded-md border border-up/35 bg-up/5 p-2.5 text-xs text-paper-muted">
                  <p className="text-paper">{gap.title}</p>
                  <p className="mt-1">
                    Buy Polymarket {gap.buyPolymarket.toUpperCase()} + Kalshi {gap.buyKalshi.toUpperCase()} · gross edge {formatPercent(gap.grossEdgePct, 2)}
                  </p>
                  <p className="mt-1">
              ...
              {data.withinVenue.map((gap) => (
                <div key={gap.url} className="rounded-md border border-up/35 bg-up/5 p-2.5 text-xs text-paper-muted">
                  <p className="text-paper">{gap.title}</p>
                  <p className="mt-1">
                    {gap.venue} YES + NO asks · gross edge {formatPercent(gap.grossEdgePct, 2)}
                  </p>
                  <p className="mt-1"><VenueLink href={gap.url}>Open order book</VenueLink></p>
                </div>
              ))}
            </div>
          ) : (
            <p className="mt-2 text-xs leading-relaxed text-paper-muted">
              No gross parity gaps above 1.0% found in the public books. Exact cross-venue pairs: {data.exactPairCount}.
            </p>
          )}

          {mismatch ? (
            <div className="mt-3 rounded-md border border-line/70 bg-ink/40 p-2.5 text-[11px] leading-relaxed text-paper-muted">
              <p className="uppercase tracking-wider text-[10px]">Closest look-alike</p>
              <p className="mt-1">
                {mismatch.polymarket ? <VenueLink href={mismatch.polymarket.url}>Polymarket</VenueLink> : "Polymarket"}
                {mismatch.polymarket ? `: ${mismatch.polymarket.settlement}.` : " contract unavailable."}
                {" "}
                {mismatch.kalshi ? <VenueLink href={mismatch.kalshi.url}>Kalshi</VenueLink> : "Kalshi"}
                {mismatch.kalshi ? `: ${mismatch.kalshi.settlement}.` : " contract unavailable."}
              </p>
              <p className="mt-1">{mismatch.reasons.join(" ")}</p>
            </div>
          ) : null}
          <p className="mt-2 text-[10px] uppercase tracking-[0.14em] text-paper-muted">
            Public books · {formatRelativeAge(data.asOf)} · fees, transfer time, and fill risk excluded
          </p>
        </>
      ) : null}
    </aside>
