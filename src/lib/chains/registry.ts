...
export const CHAINS: Record<ChainId, ChainConfig> = {
  btc: {
    ...
    shortName: "BTC",
    ticker: "BTC",
    blurb: "Ten-minute heartbeat, live dials: the original board.",
    ...
    observatoryBlurb:
      "Block timing, mempool weather, tip face, halvings, hashrate: the quiet stuff that matters. Use expand to go fullscreen. On Atmosphere, a bright dot peeks at a tx; empty space opens a bigger view.",
    instruments: {
      metronome: {
  ...
  sol: {
    ...
    observatoryBlurb:
      "A tach bay, not a ten-minute clock. Tap a jet to peek; empty space goes fullscreen. Tip time updates often. The full board refreshes about every 15 seconds.",
    instruments: {
      metronome: {
        frameTitle: "Tach",
        title: "Turbine Tach",
  ...
  hype: {
    ...
    observatoryBlurb:
      "Clock, tide, tape, fountain, vault. Tip timing is HyperEVM (~1s), not HyperCore L1, so don’t mix those up. Click Tide to stretch out; the full board refreshes about every 15 seconds.",
    instruments: {
      metronome: {
        frameTitle: "Clock",
...
export const SUITE = {
  name: "Chain Dials",
  tagline: "Live network dials for BTC, ETH, SOL, and HYPE.",
  description:
    "Live dials for Bitcoin, Ethereum, Solana, and Hyperliquid. Same layout idea on every board.",
} as const;
